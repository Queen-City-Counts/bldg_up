...
def Buffalo_OARS_data(startdate, enddate):
    ...
    source = urllib.request.urlopen(start_url)
    soup = bs.BeautifulSoup(source, 'lxml')
    last_page = int(str(soup.find_all('strong')).split(',')[3].split()[3])
    ...
    df.columns = ['address', 'sbl_long', 'sbl_short', 'sale_price', 'sale_date', 'assessment', 'prop_type', 'lot_size', 'yr_blt', 'sqft', 'bedrooms', 'baths', 'fireplaces']
    return df

startdate = '2000-01-01'
enddate = '2019-10-15'
df = Buffalo_OARS_data(startdate, enddate)
##df.to_csv ('property_sales_(' + startdate + '--' + enddate + ').csv', index = None, header=True,float_format=str)
